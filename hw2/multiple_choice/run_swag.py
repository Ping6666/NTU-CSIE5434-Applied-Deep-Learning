...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(default=None,
                                       metadata={"help": "The predict file."})
    ...
    test_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input test data file to predict."},
    )
...
def main():
    ...
    if training_args.do_predict:
        ...
        if data_args.max_eval_samples is not None:
            max_eval_samples = min(len(test_dataset),
                                   data_args.max_eval_samples)
    ...
    if training_args.do_predict:
        ...
        preds_list = []
        for t, p in tqdm(zip(test_dataset, preds), total=len(test_dataset)):
            ex = {
                'id': t['id'],
                'question': t['sent1'],
                'context': t['ending' + str(p)],
            }
            preds_list.append(ex)

        with open(data_args.output_file, 'w', encoding='utf-8') as f_json:
            json.dump(preds_list, f_json, ensure_ascii=False)
