...
def convert_simple_text2vec(a: str):
    '''
        Returns:
            text vector with shape 768
    '''
    global sentence_model
    rt = sentence_model.encode(a)
    return rt.reshape(-1)
...
def manipulate_courses(df: pd.DataFrame) -> Tuple[pd.DataFrame, LabelEncoder]:
    ...
    df['courses_simple_text2vec'] = df[['course_name']].progress_apply(
        lambda x: convert_simple_text2vec(x),
        axis=1,
    )
...
def _dataset_workhouse(name, df_preprocess) -> Tuple[List, pd.DataFrame, List]:
    ...
    df_final = pd.merge(df_users, df_course, on='user_id')
    ...
    ## train part ##
    # input / ground_truth
    df_final = df_final[['users_interests', 'label_course_id']]

    # augmented
    input_feature = np.array(df_courses['courses_simple_text2vec'].tolist())

    print('df_final', df_final)
    return ((user_id, user_id_labelencoder), df_final, input_feature,
            (course_id_labelencoder))
...
def inference_prediction(
    ...
    for predict in predicts:
        # course
        c_course_list = [idx + 1 for idx in np.argsort(-np.array(predict))]
        c_course_lists.append(c_course_list)
    return c_course_lists
